...
public struct ResizeWindow
{
	...
	public void ResizeUpF (ref Rect ResizingWindow)
	{
		...
		if (ResizeUp) {
			ResizingWindow.y = (Screen.height - Input.mousePosition.y)/AriscoGUI.GUI_SCALE - 1;
			ResizingWindow.height = rememberY - (Screen.height - Input.mousePosition.y)/AriscoGUI.GUI_SCALE;
			if (Event.current.type.ToString () == "mouseUp") {
				ResizeUp = false;
			} else if (Event.current.type.ToString () == "mouseDrag") {
	...
	public void ResizeLeftF (ref Rect ResizingWindow)
	{
		...
		if (ResizeLeft) {
			ResizingWindow.x = Input.mousePosition.x/AriscoGUI.GUI_SCALE - 1;
			ResizingWindow.width = rememberX - Input.mousePosition.x/AriscoGUI.GUI_SCALE;
			if (Event.current.type.ToString () == "mouseUp") {
				ResizeLeft = false;
			} else if (Event.current.type.ToString () == "mouseDrag") {
